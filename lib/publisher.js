...
function unpublish(postdir, unpublished) {
  ...
  function ensurePostJsonFileExists (cb) {
    utl.ensurePathExists(postjsonFile, cb);
  }

  function removePostJsonFile (cb) {
    fs.unlink(postjsonFile, cb);
  }

  function resolveCurrentBlogMetadata (cb) {
    var metadata;
  ...
  function removeUnusedTags (metadata, cb) {
    var json;
  ...
  function writeBlogJsonFile (json, cb) {
  ...
  runnel(
      ensurePostJsonFileExists
    , removePostJsonFile
    , resolveCurrentBlogMetadata
    , removeUnusedTags
    , writeBlogJsonFile
    , unpublished
  );
