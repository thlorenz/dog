...
function render(postdir, rendered) {
  ...
  var placeheld = []
    ...
    , placeholderHtml = '<h1>' + placeholder.slice(2) + '</h1>'
    , activateScriptieTalkie = [
  ...
  function placeholdScriptieTalkies(markdown, cb) {
    var detect           =  /[`]{3,4}js st/g
      , start            =  /^[`]{3,4}js +st *$/
      , end              =  /^[`]{3,4} *$/
      ;

    if (!detect.test(markdown)) cb(null, markdown);
  ...
  function insertScriptieTalkies(html, cb) {
    if (!placeheld.length) cb(null, html);
    ...
    lines.push(activateScriptieTalkie);
    cb(null, lines.join('\n'));
  }
